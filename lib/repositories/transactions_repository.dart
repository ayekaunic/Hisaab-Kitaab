    ...
    try {
      ...
      if (dbAlreadyExists) {
        ...
        _db = await sqf.openDatabase(
          ...
          onCreate: (db, version) {
            return db.execute(
                'CREATE TABLE $_tableName(id TEXT PRIMARY KEY, title TEXT, amount TEXT, date INTEGER, createdOn INTEGER)');
          },
        );
        return [];
      }
  ...
  Future<List<Transaction>> getAllTransactions() async {
    try {
      ...
      final List<Transaction> transactions = tList
          .map((tMap) => Transaction.fromMap(tMap as Map<String, dynamic>))
          .toList();
      return transactions;
    } catch (e) {
